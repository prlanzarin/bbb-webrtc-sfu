...
module.exports = class Freeswitch extends EventEmitter {
  ...
  async createMediaElement (roomId, type, params) {
    ...
    try {
      ...
      return Promise.resolve({ mediaElement: roomId, host: {ip: FREESWITCH_IP, port: FREESWITCH_PORT} });
    }
    catch (err) {
      ...
      try {
        if (userAgent) {
          ...
          if (this._rtpConverters[elementId]) {
            mediaElement = this._rtpConverters[elementId];
          }
          else {
            ({ mediaElement, host }  = await this._Kurento.createMediaElement(elementId, 'RtpEndpoint'));
            this._rtpConverters[elementId] = mediaElement;
          }

          Logger.info("[mcs-media-freeswitch] RTP endpoint equivalent to SIP instance is", mediaElement);
  ...
  _createUserAgent (type, displayName, roomId) {
    ...
    var newUA = new SIPJS.UA({
      uri: 'sip:' + C.FREESWITCH.GLOBAL_AUDIO_PREFIX + roomId + '@' + FREESWITCH_IP,
