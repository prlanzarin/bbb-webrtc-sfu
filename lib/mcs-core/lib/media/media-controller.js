...
const { sortBitrateThresholds, handleError } = require('../utils/util.js');
...
const ALLOW_DUPLICATE_EXT_USER_ID = config.has('allowDuplicateExtUserId')
  ...
  : true;
const GLOBAL_BITRATE_THRESHOLDS = sortBitrateThresholds(config.get('bitrateThresholds').global);
...
module.exports = class MediaController {
  ...
  hasBitrateThresholdChanged (mediaTypes) {
    const mediasOfType = this.medias.filter(m => Object.keys(mediaTypes).some(t => m.mediaTypes[t]));
    if (GLOBAL_BITRATE_THRESHOLDS > 0 && mediasOfType.length >= GLOBAL_BITRATE_THRESHOLDS) {
      Logger.error(LOG_PREFIX, `Server has exceeded the bitrate threshold`,
        { threshold: GLOBAL_BITRATE_THRESHOLDS, current: this.medias.length }
      );
      return true;
    }
    return false;
  ...
  checkBitrateThresholds (room, user, mediaSession) {
    const mediaTypes = mediaSession.mediaTypes;
    const has = room.hasBitrateThresholdChanged(mediaTypes);
    if (false) {
      // TODO properly implemen this.hasBitrateThresholdChanged
      room.suggestBitrateChange(mediaTypes);
      return true;
    } else if (room.hasBitrateThresholdChanged(mediaTypes)) {
      room.notifyBitrateChange(mediaTypes);
      return true;
    } else if (user.hasBitrateThresholdChanged(mediaTypes)) {
      user.notifyBitrateChange(mediaTypes);
      return true;
    } else {
      return false;
    }
  ...
  async publishAndSubscribe (roomId, userId, sourceId, type, params = {}) {
    ...
    if (!params.ignoreBitrateThreshold) {
      // TODO hook up these checks for when media is actually flowing
      // to avoid glaring stuff. Consider also adding a grace period
      this.checkBitrateThresholds(room, user, session);
    }
