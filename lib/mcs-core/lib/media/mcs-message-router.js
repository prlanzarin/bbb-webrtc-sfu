...
const MR = class MCSRouter {
  ...
  setupClient(client) {
    ...
    client.on('onEvent', async (args) => {
      try {
        Logger.info("[mcs-router] Client", client.trackingId, "subscribing to event", args);
  ...
  _dispatchEvents() {
    ...
    this.emitter.on(C.EVENT.BITRATE_CHANGE, event => {
      const { mediaSessionId, medias, sdp } = event;
      const clients = this._getClientToDispatch(mediaSessionId, C.EVENT.BITRATE_CHANGE);

      clients.forEach(client => {
        client.bitrateChange(mediaSessionId, medias, sdp);
      });
    });
  }
}
