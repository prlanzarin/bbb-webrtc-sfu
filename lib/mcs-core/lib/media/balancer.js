...
class Balancer extends EventEmitter {
  ...
  async getHost (roomId, mediaType = C.MEDIA_PROFILE.ALL) {
    Logger.info(`[mcs-balancer] Getting host for mediaType: ${mediaType}`);

    const host = this._fetchAvailableHost(roomId, mediaType);
    if (host == null) {
  ...
  _fetchAvailableHost (roomId, mediaType) {
    ...
    switch (BALANCING_STRATEGY) {
      case C.BALANCING_STRATEGY.ROOM:
        return this._roomHost(roomId);
        break;
  ...
  _roomHost (roomId) {
    let host = this.hosts.find(host => host.roomId === roomId);

    if (host == null) {
      host = this.hosts.find(host => {
        return host.roomId == null;
      });
    }

    if (host == null) {
      host = this._roundRobinHost();
    } else {
      host.roomId = roomId;
    }
