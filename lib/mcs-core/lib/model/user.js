...
const USER_BITRATE_THRESHOLDS = sortBitrateThresholds(config.get('bitrateThresholds').user);
...
module.exports = class User extends EventEmitter  {
  constructor(roomId, type, params = {}) {
    ...
    this.currentBitrateThresholds = { ...USER_BITRATE_THRESHOLDS } || bitrateThresholdDictionary();
  }
  ...
  hasBitrateThresholdChanged (mediaTypes) {
    ...
    Object.keys(mediaTypes).forEach(mediaType => {
      ...
      const nofMediasOfType = getMediaSessionsOfType(
        Object.values(this.mediaSessions),
        mediaType
      ).length;
      const bitrateThreshold = getNewBitrateThreshold(
        USER_BITRATE_THRESHOLDS[mediaType],
        nofMediasOfType,
      ...
      if (bitrateThreshold.threshold !== this.currentBitrateThresholds[mediaType][0].threshold) {
        Logger.warn(LOG_PREFIX, `User has exceeded the bitrate threshold`,
          { userId: this.id, roomId: this.roomId, threshold: USER_BITRATE_THRESHOLDS, current: this.medias.length}
        );
  ...
  suggestBitrateChange (mediaTypes) {
    Object.keys(mediaTypes).forEach(mediaType => {
      const sourceSessionsOfType = getMediaSessionsOfType(
        Object.values(this.mediaSessions),
        mediaType
      ).length;
      sourceMediaSessions.forEach((mediaSession) => {
        mediaSession.suggestBitrateChange(mediaType, this.currentBitrateThresholds[mediaType]);
      });
    });
  }
