...
const {
  bitrateThresholdDictionary,
  sortBitrateThresholds,
  getNewBitrateThreshold,
  getMediaSessionsOfType,
} = require('../utils/util.js');

const { perRoom: ROOM_MEDIA_THRESHOLD } = config.get('mediaThresholds');
const ROOM_BITRATE_THRESHOLDS = sortBitrateThresholds(config.get('bitrateThresholds').room);
...
module.exports = class Room {
  constructor (id) {
    ...
    this.currentBitrateThresholds = { ...ROOM_BITRATE_THRESHOLDS } || bitrateThresholdDictionary();
  }
  ...
  hasBitrateThresholdChanged (mediaTypes) {
    let changed = false;
    ...
    Object.keys(mediaTypes).forEach(mediaType => {
      ...
      const nofMediasOfType = getMediaSessionsOfType(this.mediaSessions, mediaType).length;
      const bitrateThreshold = getNewBitrateThreshold(
        ROOM_BITRATE_THRESHOLDS[mediaType],
        nofMediasOfType,
        this.currentBitrateThresholds[mediaType][0]
      );

      if (bitrateThreshold.threshold !== this.currentBitrateThresholds[mediaType][0].threshold) {
        Logger.warn(LOG_PREFIX, `Room has exceeded the bitrate threshold`,
          { roomId: this.id, threshold: ROOM_BITRATE_THRESHOLDS, current: this.medias.length}
        );
        this.currentBitrateThresholds[mediaType][0] = bitrateThreshold;
        changed = true;
      }
      changed = false;
    });
  ...
  suggestBitrateChange (mediaTypes) {
    Object.keys(mediaTypes).forEach(mediaType => {
      const sourceSessionsOfType = this.getSourceMediaSessionsOfType(mediaType);
      sourceSessionsOfType.forEach((mediaSession) => {
        mediaSession.suggestBitrateChange(mediaType, this.currentBitrateThresholds[mediaType][0]);
      });
    });
  }
  ...
  getSourceMediaSessions () {
    return this.mediaSessions.filter(({ medias }) => {
      medias.some(({ mediaTypes }) => {
        Object.values(mediaTypes).some((direction) => {
  ...
  getSourceMediaSessionsOfType (mediaType) {
    return this.mediaSessions.filter(({ medias }) => {
      return medias.some(({ mediaTypes }) => mediaTypes[mediaType] && mediaTypes[mediaType] !== 'recvonly');
    });
  }

  getSinkMediaSessionsOfType (mediaType) {
    return this.mediaSessions.filter(({ medias }) => {
      return medias.some(({ mediaTypes }) => mediaTypes[mediaType] && mediaTypes[mediaType] !== 'sendonly');
    });
  }
