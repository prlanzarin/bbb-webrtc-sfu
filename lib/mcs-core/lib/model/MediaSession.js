...
module.exports = class MediaSession {
  ...
  async start () {
    ...
    try {
      ...
      const { mediaElement, host } = await this._MediaServer.createMediaElement(this.room, this._type, this._options);
  ...
  async stop () {
    if (this._status === C.STATUS.STARTED) {
      ...
      try {
        ...
        if (this.hasVideo) {
          this.balancer.decrementHostStreams(this.host.id, 'video');
        }

        if (this.hasAudio) {
          this.balancer.decrementHostStreams(this.host.id, 'audio');
        }
  ...
  async connect (sinkId, type = 'ALL', shouldTranspose) {
    try {
