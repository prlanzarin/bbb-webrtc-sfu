...
module.exports = class MediaSession {
  ...
  suggestBitrateChange (mediaType, bitrateThreshold) {
    try {
      const mediasToRequest= this.medias.filter(({ mediaTypes }) => mediaTypes[mediaType]
        && mediaTypes[mediaType] !== 'recvonly');
      ...
      const event = {
        ...
        medias: [],
        descriptor: 'a', // TODO pass the wrapper bw override and return a new descriptor
      }

      mediasToRequest.forEach(async m => {
        try {
          const changed = m.suggestBitrateChange(bitrateThreshold);
          if (changed) {
            event.medias.push({
              mediaId: this.id,
              suggestedBitrate: m.tentativeCurrentBitrate,
            });
          }
        } catch (error) {
      ...
      if (event.medias.length > 0) {
        GLOBAL_EVENT_EMITTER.emit(C.EVENT.BITRATE_CHANGE, event);
      }
    } catch (error) {
