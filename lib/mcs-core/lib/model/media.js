...
module.exports = class Media extends EventEmitter {
  constructor(
    ...
    this.currentBitrateThreshold = { threshold: 0, bitrate: 0 };
    // TODO should fetch it from the wrapper.
    this.tentativeCurrentBitrate = 0;
  ...
  suggestBitrateChange (bitrateThreshold) {
    try {
      const { threshold, bitrateMultiplier } = bitrateThreshold;
      if (this.currentBitrateThreshold.threshold < threshold) {
        this.currentBitrateThreshold = bitrateThreshold;
        this.tentativeCurrentBitrate = Math.round(this.tentativeCurrentBitrate * bitrateMultiplier);
        return true;
      }
      return false;
    } catch (error) {
