...
module.exports = class Video extends BaseProvider {
  constructor(_bbbGW, _meetingId, _id, _shared, _connectionId) {
    ...
    this._bbbGW.once(C.RECORDING_STATUS_REPLY_MESSAGE_2x+this.meetingId, (payload) => {
  ...
  sendPlayStop () {
    ...
    this._bbbGW.publish(userCamEvent, function(error) {});
  ...
  sendGetRecordingStatusRequestMessage() {
    ...
    this._bbbGW.publish(req, C.TO_AKKA_APPS);
  }
  ...
  sendStartShareEvent() {
    ...
    this._bbbGW.writeMeetingKey(this.meetingId, shareCamEvent, function(error) {});
  }

  sendStopShareEvent () {
    let stopShareEvent =
      Messaging.generateWebRTCShareEvent('StopWebRTCShareEvent', this.meetingId, this.recording.filename);
    this._bbbGW.writeMeetingKey(this.meetingId, stopShareEvent, function(error) {});
  }
