...
module.exports = class VideoManager extends BaseManager {
  ...
  async _onMessage (_message) {
    ...
    if (!message.cameraId && message.id !== 'close') {
      Logger.warn(this._logPrefix, 'Ignoring message with undefined.cameraId for session', sessionId, message);
      return;
    }
      ...
      case 'start':
        ...
        try {
          ...
          video.once(C.MEDIA_SERVER_OFFLINE, async (event) => {
            ...
            this.sendToClient({ ...errorMessage }, C.FROM_VIDEO);
          });
          ...
          return this.sendToClient({ ...errorMessage }, C.FROM_VIDEO);
        }
        break;
