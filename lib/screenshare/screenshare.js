...
module.exports = class Screenshare extends BaseProvider {
  constructor(id, bbbgw, voiceBridge, userId, vh, vw, meetingId) {
    ...
    this._bbbGW.on(C.RECORDING_STATUS_REPLY_MESSAGE_2x+meetingId, (payload) => {
  ...
  sendStartShareEvent () {
    ...
    this._bbbGW.writeMeetingKey(this.recording.meetingId, shareEvent, function(error) {
  ...
  _startPresenter (sdpOffer) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let mediaServerIp = retRtp.answer.match(/(IP4\s)([0-9.]*)/g)[1];
  ...
  _stopScreensharing() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const isTranscoderAvailable = await this._bbbGW.isChannelAvailable(C.TO_BBB_TRANSCODE_SYSTEM_CHAN);
        ...
        if (isTranscoderAvailable) {
          ...
          this._bbbGW.once(C.STOP_TRANSCODER_REPLY+this._meetingId, async (payload) => {
          ...
          this._bbbGW.once(C.STOP_TRANSCODER_RESP_2x+this._meetingId, async (payload) => {
            const meetingId = payload[C.MEETING_ID_2x];
            await this._stopRtmpBroadcast(meetingId);
            return resolve();
          });

          this._bbbGW.publish(strm, C.TO_BBB_TRANSCODE_SYSTEM_CHAN, function(error) {});
        }
  ...
  async _onRtpMediaFlowing() {
    if (!this._rtmpBroadcastStarted) {
      ...
      if (isTranscoderAvailable) {
        ...
        this._bbbGW.once(C.START_TRANSCODER_REPLY+this._meetingId, (payload) => {
        ...
        // Capturing stop transcoder responses from the 2x model
        this._bbbGW.once(C.START_TRANSCODER_RESP_2x+this._meetingId, (payload) => {
  ...
  _stopRtmpBroadcast (meetingId) {
    return new Promise((resolve, reject) => {
      ...
      this._bbbGW.publish(dsrstom, C.FROM_VOICE_CONF_SYSTEM_CHAN);
      resolve();
    });
  }
  ...
  _startRtmpBroadcast (meetingId, output) {
    ...
    this._bbbGW.publish(dsrbstam, C.FROM_VOICE_CONF_SYSTEM_CHAN, function(error) {});
