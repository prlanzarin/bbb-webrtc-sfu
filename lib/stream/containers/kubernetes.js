...
const K8S_POD_URL = `/api/v1/namespaces/${jobNamespace}/pods/`;
const K8S_JOB_URL = `/apis/batch/v1/namespaces/${jobNamespace}/jobs`;
...
module.exports = class KubernetesSpawner {
  ...
  async startContainer(link, streamUrl) {
    ...
    this.jobName = (jobObject.metadata.name + this.meetingId).substring(0, 63);

    jobObject.spec.template.spec.containers[0].image = this.imageName;
    ...
    jobObject.metadata = { ...jobObject.metadata, name: this.jobName };
